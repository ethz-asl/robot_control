...
from robot_control.simulation import PybulletRobotWrapper, JointCommandVector, JointCommandType, JointCommand
...
class ManipulatorTest:
    ...
    def sim(self):
        ...
        while True:
            ...
            if user_input:
                ...
                target_rotation = d_rot.as_matrix().dot(self.initial_rotation)
            ...
            cmds = JointCommandVector(self.robot_sim.get_joint_names) # TODO (giuseppe) missing this is not working
            for tau in tau_vec:
                cmd = JointCommand()
                cmd.cmd_type = JointCommandType.TORQUE
                cmd.torque = tau
                cmds.commands.append(cmd)
            self.robot_sim.set_commands(cmds)
    ...
    def draw_trajectory(self):
        ...
        if self.has_prev_pos:
            p.addUserDebugLine(self.prev_pose, t, [1, 0, 0], 1)
        self.prev_pose = t
        self.has_prev_pos = True

    def get_contact_wrench(self):
        normal_vector_world = self.robot_sim.get_contact_wrench(self.table, "end_effector_link", -1)
