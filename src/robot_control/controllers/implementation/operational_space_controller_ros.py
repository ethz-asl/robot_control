...
from robot_control.controllers.implementation import OperationalSpaceController


class OperationalSpaceControllerRos(OperationalSpaceController):
    def __init__(self, robot, controlled_frame):
        OperationalSpaceController.__init__(self, robot, controlled_frame=controlled_frame)
    ...
    def compute_command(self):
        tau = OperationalSpaceController.compute_command(self)
        tau_no_nle = tau - self.robot.get_nonlinear_terms()

        # Visualize ee forces not related to non linear compensation
        wrench = np.dot(np.linalg.pinv(self.j.T, rcond=0.001), tau_no_nle)
        self.set_target_wrench_ros(wrench, self.controlled_frame)
